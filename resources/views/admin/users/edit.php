<?php view('admin.layouts.header', ['title' => trans('admin.users') . ' - ' . trans('admin.edit')]);
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    <div
        ...
        <h2>{{trans('admin.users')}} / {{trans('admin.edit') .' #'. $users['id']}}</h2>
    ...
    <form method="post" action="<?php echo aUrl('users/edit?id=' . $users['id']) ?>" enctype="multipart/form-data">
            ...
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <select name="user_type"
                        ...
                        <!-- set value form users['user_type'] -->
                        <option value="admin" <?php echo $users['user_type'] == 'admin' ? 'selected' : ''; ?>>{{ 'admin'
                            }}</option>
                        <option value="user" <?php echo $users['user_type'] == 'user' ? 'selected' : ''; ?>>{{ 'user' }}
                        </option>
        ...
        <input type="submit" class="btn btn-success m-3" value="{{trans('admin.save')}}" />


    </form>

    <form method="post" action="<?php echo aUrl('users/delete?id=' . $users['id']) ?>" enctype="multipart/form-data">
        <input type="hidden" name="_method" value="post" />
        <input type="submit" class="btn btn-success m-3" value="{{trans('admin.delete')}}" />
